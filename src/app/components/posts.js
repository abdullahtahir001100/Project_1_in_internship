...
export default function Posts() {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      const resDepts = await axios.get('https://myproject2.xo.je/api/Posts/get_department.php');
  ...
  const openModal = async (id = null) => {
    if (id) {
      setEditId(id);
      try {
        const res = await axios.get(`https://myproject2.xo.je/api/Posts/get_data.php?id=${id}`);
  ...
  const handleSave = async (e) => {
    ...
    try {
      const res = await axios.post(`https://myproject2.xo.je/api/Posts/${url}`, fd);
  ...
  const handleDelete = async () => {
    try {
      await axios.delete('https://myproject2.xo.je/api/Posts/delete.php', { data: { id: targetId } });
