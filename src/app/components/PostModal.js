...
export default function PostModal() {
  ...
  useEffect(() => {
    async function get_all_departments() {
      try {
        setloading(true);
        const request = await fetch('http://localhost/react-backend/api/Posts/get_department.php')
  ...
  const add_posts = async (e) => {
    ...
    try {
      setloading(true);
      const insert = await fetch('http://localhost/react-backend/api/Posts/create.php', {
        method: 'POST',
        body: formData,
      })
