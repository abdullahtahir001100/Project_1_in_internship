...
const PayrollProcessor = () => {
    ...
    async function fetchPayrollData(dateParam = null) {
        try {
            ...
            const res = await axios.get(`/api/api/bonusmain/all?duration=${dateToUse}`);
            ...
            const deptReq = await axios.get(`/api/api/Posts/get_department`);
    ...
    async function handleProcessPayroll() {
        try {
            setIsLoading(true);
            const res = await axios.post(`/api/api/payrool/create`, payload);
    ...
    async function get() {
        try {
            const res = await axios.get(`/api/api/payrool/get`);
            setgeton(res.data);
    ...
    const handleEdit = async (dateStr) => {
        if (dateStr) {
            ...
            try {
                ...
                const res = await axios.get(`/api/api/bonusmain/all?duration=${dateStr}`);
    ...
    async function handleDelete(dateStr) {
        ...
        try {
            setIsLoading(true);
            await axios.delete(`/api/api/payrool/delete?date=${dateStr}`);
